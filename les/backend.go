...
import (
	...
	rpc "github.com/ShyftNetwork/go-empyrean/rpc"
)
...
func New(ctx *node.ServiceContext, config *eth.Config) (*LightEthereum, error) {
	...
	leth := &LightEthereum{
		...
		engine:         eth.CreateConsensusEngine(ctx, chainConfig, &config.Authash, &config.Ethash, nil, false, chainDb),
		shutdownChan:   make(chan bool),
		networkId:      config.NetworkId,
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   eth.NewBloomIndexer(chainDb, shyftDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
	}
	...
	leth.chtIndexer = light.NewChtIndexer(chainDb, shyftDb, leth.odr, params.CHTFrequencyClient, params.HelperTrieConfirmations)
	leth.bloomTrieIndexer = light.NewBloomTrieIndexer(chainDb, shyftDb, leth.odr, params.BloomBitsBlocksClient, params.BloomTrieFrequency)
	...
	if leth.protocolManager, err = NewProtocolManager(leth.chainConfig, light.DefaultClientIndexerConfig, true, config.NetworkId, leth.eventMux, leth.engine, leth.peers, leth.blockchain, nil, chainDb, shyftDb, leth.odr, leth.relay, leth.serverPool, quitSync, &leth.wg); err != nil {
		return nil, err
	}
