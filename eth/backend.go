...
import (
	...
	"github.com/ShyftNetwork/go-empyrean/consensus/clique"
	...
	"github.com/ShyftNetwork/go-empyrean/miner"
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	chainConfig, genesisHash, genesisErr := core.SetupGenesisBlockWithOverride(chainDb, shyftDb, config.Genesis, config.ConstantinopleOverride)
	...
	if eth.protocolManager, err = NewProtocolManager(eth.chainConfig, config.SyncMode, config.NetworkId, eth.eventMux, eth.txPool, eth.engine, eth.blockchain, chainDb, shyftDb, config.Whitelist); err != nil {
		return nil, err
	}
