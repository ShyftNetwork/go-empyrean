...
func NewChtIndexer(db ethdb.Database, shyftdb ethdb.SDatabase, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	...
	return core.NewChainIndexer(db, rawdb.NewTable(db, "chtIndex-"), shyftdb, backend, size, confirms, time.Millisecond*100, "cht")
}
...
func NewBloomTrieIndexer(db ethdb.Database, shyftdb ethdb.SDatabase, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	...
	return core.NewChainIndexer(db, rawdb.NewTable(db, "bltIndex-"), shyftdb, backend, size, 0, time.Millisecond*100, "bloomtrie")
}
